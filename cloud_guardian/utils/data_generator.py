...
def generate_fake_iam_policies(num_nodes: int, num_resources: int, num_permissions: int, file_path: Path):
    ...
    resources = [f"{random.choice(['Datastore', 'Compute'])}{i+1}" for i in range(num_resources)]
    ...
    with open(file_path, mode='w', newline='') as file:
        ...
        for _ in range(num_permissions):
            if random.choice([True, False]):  # Decide between entity-resource or entity-entity action
                ...
                if random.choice([True, False]):  # Part_Of or Assume_Role
                    source = random.choice(users)  # Part_Of can only be from User to Group
                    target = random.choice(groups)
                    permission = "Part_Of"
                else:  # Assume_Role
                    source = random.choice(users + groups)  # Assume_Role can be from User/Group to Role
