...
def create_policy(iam, policy_name: str, policy_document: dict) -> Optional[str]:
    ...
    try:
        ...
        logger.info(f"Policy {policy_name} created with ARN: {response['Policy']['Arn']}")
...
def get_policy_document(iam, policy_arn: str) -> Optional[dict]:
    ...
    try:
        ...
        version_response = iam.get_policy_version(PolicyArn=policy_arn, VersionId=policy_version_id)
    ...
    try:
        ...
        for response in response_iterator:
            for policy in response['AttachedPolicies']:
                policies.append({'arn': policy['PolicyArn'], 'name': policy['PolicyName']})
...
def get_principals_for_role(iam, role_name: str) -> List[str]:
    ...
    try:
        ...
        principals = [statement['Principal']['ID'] for statement in policy_document['Statement']]
        return principals
    except ClientError as e:
