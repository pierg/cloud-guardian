...
def get_identity_or_resource_from_arn(arn: str, iam=None, s3=None):
    try:
        ...
        resource = parsed_arn.resource  # e.g., "bucket-name" for S3 or "user/user-name" for IAM
        ...
        if service == "iam" and iam:
            ...
            if resource_type == "user":
                ...
                raise ValueError(f"Unsupported IAM resource type '{resource_type}' in ARN")
        elif service == "s3" and s3:
            return get_bucket(s3, resource.split('/')[0])
        else:
            raise ValueError(f"Unsupported service '{service}' or service client not provided in ARN")
