...
import numpy as np
from iam_model.entities import Compute, Datastore, Entity, Resource, Role, User, Group
from iam_model.permission import IAMAction, Permission, Condition
...
class IAMGraph:
    ...
    type_index: Dict[str, set] = field(default_factory=lambda: {'entity': set(), 'resource': set()})
    ...
    def get_all_type(self, node_type: str) -> Dict[str, Union[Entity, Resource]]:
        ...
        return {node_id: data['instance'] for node_id, data in self.graph.nodes(data=True) if data.get('type') == node_type}
    ...
    def add_permission(self, source_id: str, target_id: str, permission: IAMAction, condition: Condition = None):
        """Add a permission (edge) to the graph."""
        self.graph.add_edge(source_id, target_id, permission=permission, condition=condition)
    ...
    def parse_csv_and_populate(self, csv_file_path: str):
        ...
        with open(csv_file_path, mode='r') as csv_file:
            ...
            for row in csv_reader:
                source_id, permission_str, target_id = row['SourceEntity'], row['Permission'], row['Target']
    ...
    def save_graph_pdf(self, file_path: Path):
        ...
        entity_legend = plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=entity_color, markersize=10, label='Entities')
        resource_legend = plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=resource_color, markersize=10, label='Resources')
        first_legend = plt.legend(handles=[entity_legend, resource_legend], title='Node Types', loc='upper left')
        plt.gca().add_artist(first_legend)  # Add the first legend manually


        # Drawing the nodes with their respective colors
        nx.draw_networkx_nodes(self.graph, pos, nodelist=entities, node_color=entity_color, label='Entities', node_size=300)
        nx.draw_networkx_nodes(self.graph, pos, nodelist=resources, node_color=resource_color, label='Resources', node_size=300)

        # Adjust labels to be above nodes
        label_pos = {node: (position[0], position[1] + 0.05) for node, position in pos.items()}
        nx.draw_networkx_labels(self.graph, label_pos, labels=node_labels, font_size=10, verticalalignment='bottom')

        # Draw edges with IAMAction enum values as labels
        edge_labels = {(u, v): data['permission'].value[0] for u, v, data in self.graph.edges(data=True)}

        # Separate edges based on flowEnabler attribute
        flow_enabler_edges = [(u, v) for u, v, data in self.graph.edges(data=True) if data['permission'].flowEnabler]
        other_edges = [(u, v) for u, v, data in self.graph.edges(data=True) if not data['permission'].flowEnabler]


        # Draw flowEnabler edges with red arrows
        nx.draw_networkx_edges(self.graph, pos, edgelist=flow_enabler_edges, edge_color='red', arrows=True, width=1, arrowstyle='->')

        # Draw other edges with black arrows (or any color you prefer)
        nx.draw_networkx_edges(self.graph, pos, edgelist=other_edges, edge_color='black', arrows=True)


        nx.draw_networkx_edge_labels(self.graph, pos, edge_labels=edge_labels, label_pos=0.5, font_size=9)
        ...
        edge_labels_legend = [plt.Line2D([0], [0], color='black', lw=1, label=f'{action.name}: {action.value[0]}') for action in IAMAction]
