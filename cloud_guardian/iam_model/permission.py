...
class Condition:
    ...
    def evaluate(self, context: Dict[str, Any]) -> bool:
        ...
        if self.condition_operator == "equals":
            ...
            return ipaddress.ip_address(context_value) in ipaddress.ip_network(self.condition_value)
            ...
            start_time, end_time = [datetime.strptime(t, time_format).time() for t in self.condition_value]
            return start_time <= context_time <= end_time
        else:
            raise ValueError(f"Unsupported condition operator: {self.condition_operator}")
