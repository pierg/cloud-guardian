...
from cloud_guardian.utils.loaders import extract_bucket_names, load_iam_data_into_dictionaries
...
from cloud_guardian.mockup.helpers_iam import create_policy, create_role, create_group, attach_policy_to_group, create_user, attach_policy_to_user, add_user_to_group, get_principals_for_role
...
groups_dict, policies_dict, roles_dict, users_dict = load_iam_data_into_dictionaries(data_folder / "processed")
...
class BiMap:
    ...
    def add(self, key, value):
        if not (isinstance(key, (int, str, float, tuple)) and isinstance(value, (int, str, float, tuple))):
            raise ValueError("Keys and values must be hashable types (int, str, float, tuple)")
        self.map[key] = value
...
# for policies in policies_dict['ResourceBasedPolicies']:

#     # TODO: fix
#     resource_names = extract_bucket_names(policies)
